#include <Engine/Engine_Entity.h>
#include <Engine/Engine_EventHandler.h>
#include <Engine/Engine_Vector2.h>
#include <Engine/Engine_Physics.h>
#include <math.h>
#include <stdlib.h>
#include <stdio.h>


Engine_Entity Entity_Initialise(int id)
{
    return(Engine_Entity){id};
};

void Entity_Setup(Engine_Entity* entity, int xPosition, int yPosition)
{
    entity->Position = Vector2_Initialise(xPosition-50, yPosition-50);
    entity->Velocity = Vector2_Initialise(0.0, 0.0);
    entity->BoundingBox = AABB_Initialise();
	entity->Restitution = 1.0;
	entity->Mass = 10.0;

};

void Entity_Event(Engine_Entity* entity, Engine_EventHandler* eventHandler)
{

};

void Entity_SetPosition(Engine_Entity* entity, int xPosition, int yPosition)
{
    entity->Position = Vector2_Initialise(xPosition-50, yPosition-50);
};

void Entity_SetVelocity(Engine_Entity* entity, Vector2 vector)
{
    //REplace subtrace with Add
     entity->Velocity = Vector2_ScalarMuliply(
        100, Vector2_Normailised(
            Vector2_VectorAdd(
                Vector2_VectorAdd(entity->Position, Vector2_Initialise(50, 50)), vector)));//Vector2_VectorAdd(entity->Velocity, ); 

    //printf("%.2f : %.2f\n", vector.x, vector.y);
    printf("%.2f : %.2f\n", entity->Velocity.x, entity->Velocity.y);
    printf("%.2f : %.2f\n", entity->Position.x, entity->Position.y);
    
};

void Entity_UpdateRigid(Engine_Entity* entity, float deltaTime)
{
    entity->Position = Vector2_VectorAdd(Vector2_ScalarMuliply(deltaTime, entity->Velocity), entity->Position);
    AABB_AddPosition(&entity->BoundingBox, entity->Position);
}; 

void Entity_UpdateStatic(Engine_Entity* entity, float deltaTime)
{

};

void Entity_CollisionResolve(Engine_Entity* entity1, Engine_Entity* entity2, Engine_Manifold* manifold)
{
	Vector2 relativeVelocity = Vector2_VectorSubtract(entity2->Velocity, entity1->Velocity);

	float veloictyAlongNormal = Vector2_DotProduct(relativeVelocity, manifold->Normal);
	
	if (veloictyAlongNormal < 0)
	{
		return;
	}

	float e = fmin(entity1->Restitution, entity2->Restitution);

	float j = -(1 + e) * veloictyAlongNormal;
	j /= 1.0 / entity1->Mass + 1.0 / entity2->Mass;

	Vector2 impulse = Vector2_ScalarMuliply(j, manifold->Normal);
	entity1->Velocity = Vector2_VectorSubtract(entity1->Velocity, Vector2_ScalarMuliply(1.0/entity1->Mass, impulse));
	entity2->Velocity = Vector2_VectorAdd(entity2->Velocity, Vector2_ScalarMuliply(1.0/entity2->Mass, impulse));
};

Engine_Manifold Entity_CollisionNormal(Engine_Entity* entity1, Engine_Entity* entity2)
{
	Engine_Manifold output;
	output.Hit = false;

	Vector2 mid_1 = Vector2_ScalarMuliply(0.5, Vector2_VectorAdd(entity1->BoundingBox.minVector, entity1->BoundingBox.maxVector));
	Vector2 mid_2 = Vector2_ScalarMuliply(0.5, Vector2_VectorAdd(entity2->BoundingBox.minVector, entity2->BoundingBox.maxVector));
	
	Vector2 e1 = Vector_ABS(Vector2_ScalarMuliply(0.5, Vector2_VectorSubtract(entity1->BoundingBox.maxVector, entity1->BoundingBox.minVector)));
	Vector2 e2 = Vector_ABS(Vector2_ScalarMuliply(0.5, Vector2_VectorSubtract(entity2->BoundingBox.maxVector, entity2->BoundingBox.minVector)));
	
	Vector2 direction = Vector2_VectorSubtract(mid_1, mid_2);

	float directionx = e1.x + e2.x - abs(direction.x);
	if (directionx < 0) return output;
	float directiony = e1.y + e2.y - abs(direction.y);
	if (directiony < 0) return output;

	Vector2 normal;
	float depth;
	Vector2 p;

	if (directionx < directiony) 
	{
		// x axis overlap is smaller
		depth = directionx;
		if (direction.x < 0)
		{
			normal = Vector2_Initialise(-1.0f, 0);
			p = Vector2_VectorSubtract(mid_1, Vector2_Initialise(e1.x, 0));
		}
		else
		{
			normal = Vector2_Initialise(1.0f, 0);
			p = Vector2_VectorAdd(mid_1, Vector2_Initialise(e1.x, 0));
		}
	}
	else
	{
		// y axis overlap is smaller
		depth = directiony;
		if (direction.y < 0) {
			normal = Vector2_Initialise(0, -1.0f);
			p = Vector2_VectorSubtract(mid_1, Vector2_Initialise(0, e1.y));
		}
		else
		{
			normal = Vector2_Initialise(0, 1.0f);
			p = Vector2_VectorAdd(mid_1, Vector2_Initialise(0, e1.y));
		}
	}

	output.Hit = true;
	output.HitSpot = p;
	output.Depth = depth;
	output.Normal = normal;
	return output;
};